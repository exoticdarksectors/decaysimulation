...
def mesonDecayProduction(fileNamePi, fileNameEta, fileNameRho, fileNamePhi, fileNameOmega, fileNameJpsi, NPOT, fileMass):
    ...
    ageo_upsilon = np.full(mass_upsilon.size, 0.011357) #taken from J/psi efficiency at 1e-3 GeV
...
def brem_single_hit_or_shared_v2(
        ...
        det_angle=np.arctan(0.5/40.0),   # radians
        epsilon=1.0,
    ...
    for f in files:
        ...
        # Number of trackes =  Luminosity x acceptance x epsilon^2
...
def dyProduction_for_total_production(fileNamecross, filenamedy, NPOT, totalCrossSection):
    # Import Data from efficiency files
...
if __name__ == '__main__':
    ...
    mass_ship = np.loadtxt(base + 'mship_values.txt')
    mass_ship = mass_ship[:-1]
    NPOT_dark = 1e20
    ...
    # get DY production
    dy_ship = dyProduction(base+'dy_cross_darkquest_nCTEQ15_iron.txt', base+'ageo_darkquest_nCTEQ15_iron.txt', NPOT_dark, 13e-3)
    dy_shiptot = dyProduction_for_total_production(base+'dy_cross_darkquest_nCTEQ15_iron.txt', base+'ageo_darkquest_nCTEQ15_iron.txt', NPOT_dark, 13e-3)
    ...
    brem_dir_shared = "/Users/leobailloeul/Documents/coding/Archive-dec-9-2025/DarkQuest-brem-backup"
    det_angle = np.arctan(0.5 / 40.0)
    epsilon = 1.0

    # Λp = 1.0, 1.5, 2.0 GeV (lower, central, upper)
    m_low, y_low = brem_single_hit_or_shared_v2(
        brem_dir_shared, det_angle=det_angle, epsilon=epsilon,
        lambda_idx=0, N_POT=NPOT_dark, rho=7.87, L=16.8, A=55.845
    )
    m_cent, y_cent = brem_single_hit_or_shared_v2(
        brem_dir_shared, det_angle=det_angle, epsilon=epsilon,
        lambda_idx=1, N_POT=NPOT_dark, rho=7.87, L=16.8, A=55.845
    )
    m_high, y_high = brem_single_hit_or_shared_v2(
        brem_dir_shared, det_angle=det_angle, epsilon=epsilon,
        lambda_idx=2, N_POT=NPOT_dark, rho=7.87, L=16.8, A=55.845
    )
    ...
    # central Λp = 1.5 GeV curve (dashed) on top of the band
    ...
    leg = ax.legend(loc='lower left', ncol=2, frameon=True, fancybox=True, framealpha=0.85, fontsize=12.5,
    ...
    ax.text(0.48, 0.93, r'Exposure of $10^{20}$ POT at SpinQuest' '\n'
                        r'$40$ m, $0.5$ m radius cylindrical detector',
            transform=ax.transAxes, ha='left', va='center', bbox=textbox_props, fontsize=12.5)

    # save plot
    fig.tight_layout()
    fig.savefig('MCP_production_DarkQuest.pdf', bbox_inches='tight', pad_inches=0.1)
