...
for i in range(0, 5):
    ...
    plt.title('2 layers, 80 bars/layer, ' + str(bkg) + ' background events', fontsize=25)

    plt.contour(mass, charge, total_dune/5, levels=[levels[i]], colors = 'forestgreen')
    plt.plot(10, 5, color='forestgreen', label='NPOT = 10^16')

    plt.contour(mass, charge, 2*total_dune, levels=[levels[i]], colors = '#FF7E00')
    plt.plot(10, 5, color='#FF7E00', label='NPOT = 10^17')

    plt.contour(mass, charge, 20*total_dune, levels=[levels[i]], colors = '#3B5BA5')
