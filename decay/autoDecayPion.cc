// automation of decayPion.cc
// read mcp mass table from mass.txt and sample meson decay into mcp
// save only acceptance
...
double envelope = 15; // enveloping function of ddBrPi2gxx for rejection sampling

// model parameter, like coupling constants
const double epsilon   = 1.0;
const double BrPi2gg   = 1e8; // Br(pion -> gamam gamma) 
double mchi;
...
double ddBrPi2gxx(double s, double theta) {
	return sin(theta) * pow(epsilon, 2) * alpha / (4*PI*s) * pow(1-s/pow(mpi, 2), 3) * sqrt(1-4*pow(mchi, 2)/s) * (2 - (1 - 4*pow(mchi, 2)/s) * pow(sin(theta), 2)) * BrPi2gg;
}
	...
	for (int i = 0; i < nMass; i++) {
		...
		while (pionTxt >> momP >> momTheta >> momPhi) {
			...
			if (abs(y) < 0.25 && abs(z) < 0.2)
				hitCounter100m++;
            indexCounter++;
            if (indexCounter % 100000 == 0) cout << "Now at " << indexCounter << endl;
		}
